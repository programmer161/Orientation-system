...
public interface SysRoleTableDao extends BaseMapper<SysRoleTable> {
    ...
    @Select("select * from sys_role_table")
    List<SysRoleTable> getRoleAll();//获取所有角色信息

    @Select("select college,major,classes from sys_role_table where role_id = #{roleId}")
    SysRoleTable getCollegeMC(String roleId);//通过角色id获取角色管理的学院专业班级信息
    @Update("update sys_role_table set college=#{college},major=#{major},classes=#{classes} where role_id=#{roleId}")
